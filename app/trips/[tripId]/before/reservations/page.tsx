...
export default function ReservationsPage() {
  ...
  const { state, addReservation, updateReservation, removeReservation, addReservationCategory, renameReservationCategory, removeReservationCategory } = useTripData();
  ...
  const close = () => { setOpen(false); setEditingId(null); setForm(blank); setFiles([]); };
  const openNew = () => { setEditingId(null); setForm(blank); setFiles([]); setOpen(true); };
  ...
  const save = async (event: FormEvent) => {
    ...
    if (!form.date || (form.type === "Stay" && !form.endDate) || (form.type === "Flight" && (!form.endDate || !form.departureLocation || !form.arrivalLocation))) return;
    const reservation = { ...form, title: form.type === "Flight" ? flightTitle : form.title.trim(), cost: 0 };
    if (!reservation.title) return;
  ...
  const editReservation = (item: (typeof state.reservations)[number]) => { setEditingId(item.id); setForm({ ...blank, ...item, date: dateValue(item.date), endDate: dateValue(item.endDate), time: timeValue(item.time), departureTime: timeValue(item.departureTime), arrivalTime: timeValue(item.arrivalTime) }); setFiles([]); setOpen(true); };
