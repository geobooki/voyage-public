...
export default function ReservationsPage() {
  ...
  const sortedReservations = [...state.reservations].sort((a, b) => {
    const aDate = dateValue(a.date) || "9999-12-31";
    const bDate = dateValue(b.date) || "9999-12-31";
    if (aDate !== bDate) return aDate.localeCompare(bDate);
    const aTime = timeValue(a.departureTime || a.time) || "99:99";
    const bTime = timeValue(b.departureTime || b.time) || "99:99";
    return aTime.localeCompare(bTime);
  });
  const reservationDates = [...new Set(sortedReservations.map((item) => dateValue(item.date)).filter(Boolean))];
  const visibleReservations = selectedDate === "all"
    ? sortedReservations
    : sortedReservations.filter((item) => {
        const start = dateValue(item.date);
        const end = dateValue(item.endDate) || start;
        return Boolean(start) && selectedDate >= start && selectedDate <= end;
      });
