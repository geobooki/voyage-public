...
export default function ActualBudgetPage() {
  ...
  const totalKrw = useMemo(
    () =>
      state.expenses.reduce(
        (sum, item) => sum + item.amount * (rates[item.currency] || 0),
        0,
      ),
    [rates, state.expenses],
  );
  return (
    <main className="min-h-screen px-5 py-8 pb-16 sm:px-10 lg:px-20 lg:py-12">
      <div className="mx-auto max-w-4xl">
        ...
        >
          ← {ko ? "예산 계획" : "Budget plan"}
        </Link>
        <p className="eyebrow mt-10 mb-3">
          {ko ? "실제 사용" : "Actual spending"}
        </p>
        <h1 className="text-4xl font-bold">
          {ko ? "실제로 사용한 비용" : "What you actually spent"}
        </h1>
        <p className="mt-2 muted">
          {ko
            ? "여행 중 기록한 지출을 예산과 구분해 확인하세요."
        ...
        <section className="mt-9 grid gap-5 md:grid-cols-3">
          ...
          <div className="card p-6">
            <p className="eyebrow mb-2">
              {ko ? "통화별 실제 사용액" : "Actual by currency"}
            </p>
            <div className="space-y-2">
              {Object.entries(totals).length ? (
                ...
                <p className="text-sm muted">
                  {ko ? "기록된 지출이 없습니다." : "No expenses recorded."}
                </p>
              )}
            </div>
          </div>
        ...
        <section className="card mt-5 p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="eyebrow mb-2">
                {ko ? "지출 목록" : "Expense list"}
              </p>
              <h2 className="text-xl font-bold">
                {ko ? "사용 내역" : "Spending history"}
              </h2>
            </div>
            <Link
