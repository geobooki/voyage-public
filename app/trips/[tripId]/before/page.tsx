...
export default function BeforePage() {
  ...
  const categories = useMemo(
    () => [...new Set(state.packing.map((item) => item.category))],
    [state.packing],
  );
  const completed = state.packing.filter((item) => item.checked);
  const visible = state.packing.filter(
    (item) => showCompleted || !item.checked,
  );
  const submitItem = (event: FormEvent) => {
    event.preventDefault();
    if (!newItem.trim()) return;
    addChecklist("packing", { name: newItem.trim(), category, checked: false });
    setNewItem("");
  };
          ...
          <section className="card p-6 sm:p-7">
            ...
            <div className="mt-5 border-t border-[var(--color-border)] pt-5">
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {categories.map((item) => (
                  <span
                    className="inline-flex items-center gap-1 rounded-full bg-[var(--color-surface-muted)] px-3 py-1.5 text-xs font-bold"
                    key={item}
                  >
                    {item}
                    <button
                      ...
                      onClick={() => {
                        const next = window
                          ...
                          ?.trim();
                        if (next && next !== item)
                          renameChecklistCategory("packing", item, next);
                      }}
                      aria-label={ko ? `${item} 수정` : `Edit ${item}`}
                    >
                      ✎
                    </button>
                    <button
                      type="button"
                      onClick={() => removeChecklistCategory("packing", item)}
                      className="text-[var(--color-danger)]"
                      aria-label={ko ? `${item} 삭제` : `Delete ${item}`}
                    >
                      ×
                    </button>
                  </span>
