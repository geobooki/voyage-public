...
type QuickReservation = { title: string; type: string; date: string; endDate: string; location: string; departureLocation: string; arrivalLocation: string; departureTime: string; arrivalTime: string; airline: string; terminal: string; reservationNumber: string; memo: string; link: string };
const blankQuickReservation: QuickReservation = { title: "", type: "Flight", date: "", endDate: "", location: "", departureLocation: "", arrivalLocation: "", departureTime: "", arrivalTime: "", airline: "", terminal: "", reservationNumber: "", memo: "", link: "" };
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section data-section="trip-reservation-summary" className="mt-7 grid gap-4 md:grid-cols-2">
          <article className="card p-5">
            ...
            {flights.length ? flights.map((item) => <div key={item.id}><h2 className="font-bold">{item.airline || item.title}</h2><p className="mt-2 text-sm muted">{dateLabel(item.date)}{item.terminal ? ` · ${item.terminal}` : ""}</p><p className="mt-2 flex min-w-0 items-center gap-2 text-xs muted"><span className="min-w-0 flex-1 truncate">{item.departureLocation || item.location || "—"}{item.departureTime ? ` · ${item.departureTime}` : ""}</span><span aria-hidden="true">→</span><span className="min-w-0 flex-1 truncate text-right">{item.arrivalLocation || "—"}{item.arrivalTime ? ` · ${item.arrivalTime}` : ""}</span></p><ReservationDocuments tripId={tripId} reservationId={item.id} /></div>) : <p className="text-sm muted">{ko ? "항공편 예약을 추가하면 표시돼요." : "Add a flight reservation to see it here."}</p>}
          </article>
          <article className="card p-5">
        ...
        <section data-section="trip-schedule-board" className="card mt-7 p-6 sm:p-8">
          ...
          {scheduleDates.length ? <><div className="mt-6 flex items-center justify-between gap-3"><button type="button" onClick={() => setSchedulePage((current) => Math.max(0, current - 1))} disabled={schedulePage === 0} className="grid size-9 place-items-center rounded-full border border-[var(--color-border)] text-lg font-bold disabled:opacity-30" aria-label={ko ? "이전 3일" : "Previous three days"}>‹</button><p className="text-xs font-bold muted">{schedulePage * 3 + 1}–{Math.min((schedulePage + 1) * 3, scheduleDates.length)} / {scheduleDates.length}{ko ? "일" : " days"}</p><button type="button" onClick={() => setSchedulePage((current) => Math.min(schedulePageCount - 1, current + 1))} disabled={schedulePage >= schedulePageCount - 1} className="grid size-9 place-items-center rounded-full border border-[var(--color-border)] text-lg font-bold disabled:opacity-30" aria-label={ko ? "다음 3일" : "Next three days"}>›</button></div><TripScheduleCalendar dates={visibleScheduleDates} schedule={state.schedule} reservations={state.reservations} ko={ko} onAdd={openSchedule} /></> : <button type="button" onClick={() => openSchedule(trip.startDate || new Date().toISOString().slice(0, 10))} className="mt-6 w-full rounded-xl border border-dashed border-[var(--color-border)] p-6 text-sm font-bold text-[var(--color-primary)]">+ {ko ? "첫 일정 추가" : "Add your first plan"}</button>}
        </section>
        ...
        {quickReservationOpen && <div className="fixed inset-0 z-50 grid place-items-center bg-black/35 px-5 py-6"><div className="card max-h-[90vh] w-full max-w-xl overflow-y-auto p-6 sm:p-8" role="dialog" aria-modal="true" aria-labelledby="quick-reservation-title"><div className="flex items-start justify-between gap-4"><div><p className="eyebrow mb-2">{ko ? "예약 추가" : "Add reservation"}</p><h2 id="quick-reservation-title" className="text-2xl font-bold">{quickReservation.type === "Flight" ? (ko ? "항공편 예약" : "Flight reservation") : (ko ? "숙박 예약" : "Stay reservation")}</h2></div><button type="button" onClick={() => setQuickReservationOpen(false)} className="text-2xl" aria-label={ko ? "닫기" : "Close"}>×</button></div><form onSubmit={saveQuickReservation} className="mt-6 grid gap-3 sm:grid-cols-2"><label className="text-xs font-bold sm:col-span-2">{ko ? "예약 이름" : "Reservation name"}<input required value={quickReservation.title} onChange={(event) => setQuickReservation({ ...quickReservation, title: event.target.value })} placeholder={quickReservation.type === "Flight" ? "Vietnam Airlines" : (ko ? "예: 다낭 호텔" : "e.g. Da Nang hotel")} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><DateField label={ko ? "예약일" : "Reservation date"} required value={quickReservation.date} onChange={(value) => setQuickReservation({ ...quickReservation, date: value })} />{quickReservation.type === "Flight" && <><label className="text-xs font-bold">{ko ? "항공사" : "Airline"}<input value={quickReservation.airline} onChange={(event) => setQuickReservation({ ...quickReservation, airline: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "터미널" : "Terminal"}<input value={quickReservation.terminal} onChange={(event) => setQuickReservation({ ...quickReservation, terminal: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "출발지" : "Departure"}<input required value={quickReservation.departureLocation} onChange={(event) => setQuickReservation({ ...quickReservation, departureLocation: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "도착지" : "Arrival"}<input required value={quickReservation.arrivalLocation} onChange={(event) => setQuickReservation({ ...quickReservation, arrivalLocation: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "출발시간" : "Departure time"}<input required type="time" value={quickReservation.departureTime} onChange={(event) => setQuickReservation({ ...quickReservation, departureTime: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "도착시간" : "Arrival time"}<input required type="time" value={quickReservation.arrivalTime} onChange={(event) => setQuickReservation({ ...quickReservation, arrivalTime: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label></>}<label className="text-xs font-bold">{ko ? "장소" : "Location"}<input value={quickReservation.location} onChange={(event) => setQuickReservation({ ...quickReservation, location: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold">{ko ? "예약번호" : "Confirmation number"}<input value={quickReservation.reservationNumber} onChange={(event) => setQuickReservation({ ...quickReservation, reservationNumber: event.target.value })} className="mt-2 w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><label className="text-xs font-bold sm:col-span-2">{ko ? "메모" : "Memo"}<textarea value={quickReservation.memo} onChange={(event) => setQuickReservation({ ...quickReservation, memo: event.target.value })} className="mt-2 min-h-20 w-full resize-none rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-3 py-2.5 text-sm" /></label><div className="flex justify-end gap-2 border-t border-[var(--color-border)] pt-4 sm:col-span-2"><button type="button" onClick={() => setQuickReservationOpen(false)} className="rounded-xl border border-[var(--color-border)] px-4 py-3 text-sm font-bold">{ko ? "취소" : "Cancel"}</button><button className="rounded-xl bg-[var(--color-primary)] px-5 py-3 text-sm font-bold text-white">{ko ? "저장" : "Save"}</button></div></form></div></div>}
