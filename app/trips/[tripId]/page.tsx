...
type QuickReservation = { title: string; type: string; date: string; location: string; departureLocation: string; arrivalLocation: string; departureTime: string; arrivalTime: string; airline: string; terminal: string; reservationNumber: string; memo: string; link: string };
const blankQuickReservation: QuickReservation = { title: "", type: "Flight", date: "", location: "", departureLocation: "", arrivalLocation: "", departureTime: "", arrivalTime: "", airline: "", terminal: "", reservationNumber: "", memo: "", link: "" };
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <section data-section="trip-reservation-summary" className="mt-7 grid gap-4 md:grid-cols-2">
          <article className="card p-5">
            ...
            {flights.length ? flights.map((item) => <div key={item.id}><h2 className="font-bold">{item.airline || item.title}</h2><p className="mt-2 text-sm muted">{dateLabel(item.date)}{item.departureTime || item.arrivalTime ? ` · ${item.departureTime || "—"} → ${item.arrivalTime || "—"}` : item.time ? ` · ${item.time}` : ""}{item.terminal ? ` · ${item.terminal}` : ""}</p><p className="mt-1 text-xs muted">{item.departureLocation || item.location || "—"} → {item.arrivalLocation || "—"}</p><ReservationDocuments tripId={tripId} reservationId={item.id} /></div>) : <p className="text-sm muted">{ko ? "항공편 예약을 추가하면 표시돼요." : "Add a flight reservation to see it here."}</p>}
          </article>
          <article className="card p-5">
