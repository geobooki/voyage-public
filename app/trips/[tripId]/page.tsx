...
import type { DashboardItem, Reservation, ScheduleItem } from "@/types/trip";
...
function TripPageContent() {
  ...
  const { state, addExpense, addReservation, updateReservation, addSchedule, updateSchedule, addDashboardItem, updateDashboardItem, removeDashboardItem } = useTripData();
  ...
  const [editingQuickReservationId, setEditingQuickReservationId] = useState<string | null>(null);
  ...
  const [quickExpense, setQuickExpense] = useState({ name: "", amount: "", currency: "KRW", category: "", date: localDateIso(new Date()), memo: "" });
  const [scheduleOpen, setScheduleOpen] = useState(false);
  const [editingScheduleId, setEditingScheduleId] = useState<string | null>(null);
  const [scheduleDraft, setScheduleDraft] = useState({ date: "", time: "09:00", title: "", type: "관광", note: "" });
  const [wishDraft, setWishDraft] = useState<{ kind: DashboardItem["kind"]; title: string; detail: string }>({ kind: "do", title: "", detail: "" });
  const [wishNotice, setWishNotice] = useState("");
  const wishNoticeTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  const openQuickReservation = (type: "Flight" | "Stay") => {
    setEditingQuickReservationId(null);
  ...
  const saveQuickExpense = (event: FormEvent) => {
    ...
    if (!quickExpense.amount || !quickExpense.date) return;
    addExpense({ name: quickExpense.name.trim() || undefined, amount: Number(quickExpense.amount), currency: quickExpense.currency, category: quickExpense.category || expenseCategories[0] || "기타", memo: quickExpense.memo.trim(), date: quickExpense.date, time: new Date().toTimeString().slice(0, 5) });
    setQuickExpense({ name: "", amount: "", currency: quickExpense.currency, category: quickExpense.category, date: localDateIso(new Date()), memo: "" });
    setQuickExpenseOpen(false);
  };
  const editQuickReservation = (item: Reservation) => {
    setEditingQuickReservationId(item.id);
    setQuickReservation({
      ...blankQuickReservation,
      ...item,
      type: item.type,
      title: item.title || "",
      date: item.date || "",
  ...
  const saveQuickReservation = async (event: FormEvent) => {
    ...
    const reservationId = editingQuickReservationId
      ? editingQuickReservationId
      : addReservation(payload);
    if (editingQuickReservationId) updateReservation(editingQuickReservationId, payload);
    try {
  ...
  const editSchedule = (item: ScheduleItem) => {
    ...
    setScheduleDraft({ date: item.date, time: item.time || "09:00", title: item.title, type: item.type, note: item.note || "" });
    setScheduleOpen(true);
  };
  const saveSchedule = (event: FormEvent) => {
    event.preventDefault();
    if (!scheduleDraft.title.trim() || !scheduleDraft.date) return;
    const value = { ...scheduleDraft, title: scheduleDraft.title.trim() };
    if (editingScheduleId) {
      const current = state.schedule.find((item) => item.id === editingScheduleId);
      updateSchedule(editingScheduleId, { ...value, completed: current?.completed ?? false });
    } else {
      addSchedule({ ...value, completed: false });
    }
    setEditingScheduleId(null);
    setScheduleOpen(false);
  };
  const saveWish = (event: FormEvent) => {
    event.preventDefault();
    if (!wishDraft.title.trim()) return;
    addDashboardItem({ kind: wishDraft.kind, title: wishDraft.title.trim(), detail: wishDraft.detail.trim() || undefined, completed: false });
    setWishDraft((current) => ({ ...current, title: "", detail: "" }));
    setWishNotice("추가완료");
    if (wishNoticeTimer.current) clearTimeout(wishNoticeTimer.current);
    wishNoticeTimer.current = setTimeout(() => setWishNotice(""), 2200);
  };
  const toggleWish = (item: DashboardItem) => {
    updateDashboardItem(item.id, { kind: item.kind, title: item.title, detail: item.detail, url: item.url, category: item.category, completed: !item.completed });
    setWishNotice("수정 완료");
    if (wishNoticeTimer.current) clearTimeout(wishNoticeTimer.current);
    wishNoticeTimer.current = setTimeout(() => setWishNotice(""), 2200);
  };
  ...
  return (
    ...
    >
      <div className="mx-auto max-w-6xl">
          ...
          {scheduleDates.length ? (
            <>
              ...
              <div className="hidden md:block">
                <TripScheduleCalendar dates={visibleScheduleDates} schedule={state.schedule} reservations={state.reservations} ko={ko} onAdd={openSchedule} onReservationClick={editQuickReservation} onScheduleClick={editSchedule} />
              </div>
              <div className="md:hidden">
                <TripScheduleCalendar dates={[scheduleDates[mobileSchedulePage]]} schedule={state.schedule} reservations={state.reservations} ko={ko} onAdd={openSchedule} onReservationClick={editQuickReservation} onScheduleClick={editSchedule} />
              </div>
            </>
        ...
        <section data-section="trip-wish-board" className="card mt-7 p-6 sm:p-8">
          <div className="flex flex-wrap items-end justify-between gap-3">
            <div>
              <p className="eyebrow mb-2">{ko ? "여행 버킷리스트" : "Trip wishlist"}</p>
              <h2 className="text-2xl font-bold">{ko ? "가고 싶은 곳, 하고 싶은 것, 사고 싶은 것" : "Places, experiences, and things to bring home"}</h2>
              <p className="mt-2 text-sm muted">{ko ? "여행 중 체크하면 실제로 한 일과 남은 일을 한눈에 볼 수 있어요." : "Check items off during the trip to see what is done and what remains."}</p>
            </div>
            <span className="text-sm font-bold text-[var(--color-primary)]">{state.dashboardItems.filter((item) => ["do", "eat", "souvenir"].includes(item.kind) && item.completed).length} / {state.dashboardItems.filter((item) => ["do", "eat", "souvenir"].includes(item.kind)).length} {ko ? "완료" : "done"}</span>
          </div>
          <div className="mt-6 grid gap-4 md:grid-cols-3">
            {([
              ["do", ko ? "가고 싶은 곳" : "Places to go", "📍"],
              ["eat", ko ? "하고 싶은 것" : "Things to do", "✨"],
              ["souvenir", ko ? "사고 싶은 것" : "Things to buy", "🛍️"],
            ] as const).map(([kind, label, emoji]) => {
              const items = state.dashboardItems.filter((item) => item.kind === kind);
              return <div key={kind} className="rounded-2xl bg-[var(--color-background)] p-4"><p className="text-xs font-bold muted">{emoji} {label}</p><div className="mt-3 space-y-2">{items.map((item) => <div key={item.id} className={`flex items-start gap-2 rounded-xl bg-[var(--color-surface)] px-3 py-2.5 ${item.completed ? "opacity-60" : ""}`}><button type="button" onClick={() => toggleWish(item)} className="mt-0.5 shrink-0 text-base" aria-label={item.completed ? (ko ? "완료 취소" : "Mark as incomplete") : (ko ? "완료 처리" : "Mark as done")}>{item.completed ? "☑" : "□"}</button><button type="button" onClick={() => toggleWish(item)} className={`min-w-0 flex-1 text-left text-sm font-bold ${item.completed ? "line-through" : ""}`}>{item.title}{item.detail && <span className="mt-1 block text-xs font-normal muted">{item.detail}</span>}</button><button type="button" onClick={() => removeDashboardItem(item.id)} className="text-sm text-[var(--color-danger)]" aria-label={ko ? "항목 삭제" : "Delete item"}>×</button></div>)}</div>{!items.length && <p className="mt-3 text-xs muted">{ko ? "아직 추가된 항목이 없어요." : "Nothing added yet."}</p>}</div>;
            })}
          </div>
          <form onSubmit={saveWish} className="mt-5 grid gap-2 border-t border-[var(--color-border)] pt-5 md:grid-cols-[auto_1fr_1fr_auto]">
            <select value={wishDraft.kind} onChange={(event) => setWishDraft({ ...wishDraft, kind: event.target.value as DashboardItem["kind"] })} className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 text-sm outline-none focus:border-[var(--color-primary)]"><option value="do">{ko ? "가고 싶은 곳" : "Place to go"}</option><option value="eat">{ko ? "하고 싶은 것" : "Thing to do"}</option><option value="souvenir">{ko ? "사고 싶은 것" : "Thing to buy"}</option></select>
            <input required value={wishDraft.title} onChange={(event) => setWishDraft({ ...wishDraft, title: event.target.value })} placeholder={ko ? "항목 이름" : "Item name"} className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 text-sm outline-none focus:border-[var(--color-primary)]" />
            <input value={wishDraft.detail} onChange={(event) => setWishDraft({ ...wishDraft, detail: event.target.value })} placeholder={ko ? "메모 (선택)" : "Note (optional)"} className="w-full rounded-xl border border-[var(--color-border)] bg-[var(--color-background)] px-4 py-3 text-sm outline-none focus:border-[var(--color-primary)]" />
            <button className="rounded-xl bg-[var(--color-primary)] px-4 py-3 text-sm font-bold text-white">{ko ? "추가" : "Add"}</button>
          </form>
        </section>
        {wishNotice && <div className="fixed bottom-20 left-1/2 z-50 -translate-x-1/2 rounded-xl bg-gray-500/55 px-5 py-3 text-sm font-semibold text-black shadow-sm backdrop-blur-sm" role="status" aria-live="polite">{wishNotice}</div>}
