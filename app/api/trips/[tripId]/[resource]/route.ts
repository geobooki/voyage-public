...
function toRow(resource: string, tripId: string, body: Record<string, unknown>) {
  ...
  if (resource === "schedule") return { ...base, place_id: body.placeId ?? null, date: body.date ?? "2026-09-10", time: body.time ?? null, type: body.type ?? "other", title: body.title, note: body.note ?? null, sort_order: Number(body.order ?? 0) };
  ...
  if (resource === "reservations") return { ...base, type: body.type ?? "other", title: body.title, date: body.date || null, time: body.time || null, location: body.location ?? null, reservation_number: body.reservationNumber ?? null, cost: Number(body.cost ?? 0), memo: body.memo ?? null, link: body.link ?? null };
  if (resource === "budget") return { ...base, category: body.category ?? "Other", estimated_amount: Number(body.amount ?? body.estimatedAmount ?? 0), currency: body.currency ?? "KRW" };
