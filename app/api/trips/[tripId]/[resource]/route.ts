...
function toRow(resource: string, tripId: string, body: Record<string, unknown>) {
  ...
  if (resource === "reservations") return { ...base, type: body.type ?? "other", title: body.title, date: body.date || null, end_date: body.endDate || null, time: body.time || null, location: body.location ?? null, airline: body.airline ?? null, terminal: body.terminal ?? null, reservation_number: body.reservationNumber ?? null, cost: Number(body.cost ?? 0), memo: body.memo ?? null, link: body.link ?? null };
