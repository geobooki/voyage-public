...
function toRow(resource: string, tripId: string, body: Record<string, unknown>) {
  ...
  if (resource === "dashboard") return { ...base, kind: body.kind ?? "do", title: body.title, detail: body.detail ?? null, category: body.category ?? null, url: body.url ?? null, completed: Boolean(body.completed) };
