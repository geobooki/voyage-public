...
export async function GET(_request: Request, { params }: { params: Promise<{ token: string }> }) {
  ...
  const { data: share, error: shareError } = await supabase.from("trip_shares").select("trip_id, permission, invite_email").eq("token", token).is("revoked_at", null).maybeSingle();
  ...
  const [tripResult, placesResult, scheduleResult, budgetResult, checklistResult] = await Promise.all([
    ...
    supabase.from("schedule_items").select("id, date, time, title, type, completed").eq("trip_id", share.trip_id).order("date").order("time"),
  ...
  return NextResponse.json({ trip: tripResult.data, permission: share.permission, inviteEmail: share.invite_email, places: placesResult.data ?? [], schedule: scheduleResult.data ?? [], budget: budgetResult.data ?? [], checklist: checklistResult.data ?? [] });
}
