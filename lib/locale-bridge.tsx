  ...
  useEffect(() => {
    const translate = () => {
      ...
      while ((node = walker.nextNode())) {
        ...
        node.nodeValue = language === "ko" && value && ko[value] ? source.replace(value, ko[value]) : source;
      }
      document.querySelectorAll<HTMLElement>("input[placeholder], textarea[placeholder]").forEach((element) => { if (!originalPlaceholder.has(element)) originalPlaceholder.set(element, element.getAttribute("placeholder") || ""); const value = originalPlaceholder.get(element) || ""; element.setAttribute("placeholder", language === "ko" && ko[value] ? ko[value] : value); });
    };
    translate();
